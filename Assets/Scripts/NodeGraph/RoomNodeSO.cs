...
public class RoomNodeSO : ScriptableObject
{
    ...
    public void Draw(GUIStyle nodeStyle)
    {
        ...
        //if the room node has a parent or is of type entrance then display a label else display a group
        ...
        {
            //display popup using the RoomNodeType name values that can be slected from (defualt to the currently set RoomNodeType)
                ...
                if (childRoomNodeIDList.Count > 0)
                {
                    for (int i = childRoomNodeIDList.Count - 1; i >= 0; i--)
                    {
                        ...
                        if (childRoomNode != null)
                        {
                            ...
                            //remove parentID from child room 
    ...
    //<summmary>
    //Check if child node can be validly added to the parent node - return true if it can otherwise return false
    //<summary>
    public bool IsChildRoomValid(string childID)
    {
        ...
        //If the node already has child with this child ID return false
        ...
        //If this childID already has a parent  return false
        ...
        //If child is not a corridor and this node is not a corridor return false - rooms must be connected by corridors
        if (!roomNodeGraph.GetRoomNode(childID).roomNodeType.isCorridor && !roomNodeType.isCorridor)
            return false;
